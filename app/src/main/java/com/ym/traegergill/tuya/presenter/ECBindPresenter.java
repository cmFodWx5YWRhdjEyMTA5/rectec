...
import com.ym.traegergill.tuya.model.DeviceBindModel;
...
public class ECBindPresenter extends BasePresenter {
    ...
    public ECBindPresenter(Context context, IECBindView view) {
        ...
        mContext = context;
        mView = view;
        mConfigMode = ((Activity) mContext).getIntent().getIntExtra(SetUpWifiDeviceActivity.CONFIG_MODE, SetUpWifiDeviceActivity.EC_MODE);
        mModel = new DeviceBindModel(context, mHandler);
        mPassWord = ((Activity) mContext).getIntent().getStringExtra(SetUpWifiDeviceActivity.CONFIG_PASSWORD);
        mSSId = ((Activity) mContext).getIntent().getStringExtra(SetUpWifiDeviceActivity.CONFIG_SSID);
        showConfigDevicePage();
        getTokenForConfigDevice();
    }

    private void showConfigDevicePage() {
        if (mConfigMode == SetUpWifiDeviceActivity.EC_MODE) {
            mView.hideSubPage();
        } else {
            mView.hideMainPage();
            mView.showSubPage();
    ...
    public void reStartEZConfig() {
        //ap模式重试,要重新切回到EC模式里
        if (mConfigMode == SetUpWifiDeviceActivity.AP_MODE) {
    ...
    private void goToEZActivity() {
        Intent intent = new Intent(mContext, SetUpWifiDeviceActivity.class);
