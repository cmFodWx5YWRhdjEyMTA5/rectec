...
import com.ym.traegergill.activity.SetUpWifiDeviceActivity;
import com.ym.traegergill.activity.TryConnectDeviceActivity;
import com.ym.traegergill.tools.OUtil;
import com.ym.traegergill.tuya.view.IECView;
...
public class ECPresenter extends BasePresenter {
    ...
    public ECPresenter(Activity activity, IECView view) {
        mActivity = activity;
        mView = view;
        mMode = activity.getIntent().getIntExtra(SetUpWifiDeviceActivity.CONFIG_MODE, SetUpWifiDeviceActivity.EC_MODE);
       // initWifi();
    }
    ...
    public void goNextStep() {
        ...
        if (!NetworkUtil.isNetworkAvailable(mActivity) || TextUtils.isEmpty(ssid)) {
            ...
            OUtil.toastSuccess(mActivity, mActivity.getResources().getString(R.string.connect_phone_to_network));
        } else {
            //对密码进行加密处理
    ...
    private void gotoBindDeviceActivity(String ssid, String passWord) {
        Intent intent = new Intent(mActivity, TryConnectDeviceActivity.class);
        intent.putExtra(SetUpWifiDeviceActivity.CONFIG_PASSWORD, passWord);
        intent.putExtra(SetUpWifiDeviceActivity.CONFIG_SSID, ssid);
        intent.putExtra(SetUpWifiDeviceActivity.CONFIG_MODE, mMode);
