...
public class TraegerGillService1 extends Service {
    private final static String TAG = TraegerGillService1.class.getSimpleName();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        mTuyaDevice.registerDevListener(new IDevListener() {
            ...
            public void onStatusChanged(String s, boolean b) {
                sendDeviceBroadcast(TraegerGillBroadcastHelper.ACTION_DEVICE_onStatusChanged);
            }

            @Override
            public void onNetworkStatusChanged(String s, boolean b) {
                sendDeviceBroadcast(TraegerGillBroadcastHelper.ACTION_DEVICE_onNetworkStatusChanged);
            }
        ...
        switch (schema) {
            ...
            case "103":
                if (data.isFlag()) {
                    ...
                    if (Actual_temp >= minTemp && Actual_temp <= maxTemp) {
                        ...
                        sendBroadcast(TraegerGillBroadcastHelper.ACTION_ALARMS_ACTUAL_OK);
                    } else {
            ...
            case "105":
                ...
                if (data.isA_temp_open()) {
                    ...
                    if(Food_temp1<minTemp){
                        OUtil.TLog(TAG + "  tempA less");
                        sendBroadcast(TraegerGillBroadcastHelper.ACTION_ALARMS_TEMPA_LESS);
                    }else if(Food_temp1>maxTemp){
                        OUtil.TLog(TAG + "  tempA high");
                        sendBroadcast(TraegerGillBroadcastHelper.ACTION_ALARMS_TEMPA_MORE);
                    }else{
                        sendBroadcast(TraegerGillBroadcastHelper.ACTION_ALARMS_TEMPA_OK);
            ...
            case "106":
                ...
                if (data.isB_temp_open()) {
                    ...
                    if(Food_temp2<minTemp){
                        OUtil.TLog(TAG + "  tempA less");
                        sendBroadcast(TraegerGillBroadcastHelper.ACTION_ALARMS_TEMPB_LESS);
                    }else if(Food_temp2>maxTemp){
                        OUtil.TLog(TAG + "  tempA high");
                        sendBroadcast(TraegerGillBroadcastHelper.ACTION_ALARMS_TEMPB_MORE);
                    }else{
                        sendBroadcast(TraegerGillBroadcastHelper.ACTION_ALARMS_TEMPB_OK);
                        OUtil.TLog(TAG + "  tempA ok");
                    }
